...
var cache: any = {}
var exp = {
  entry: join(__dirname, 'modules/exchange/templates/*.html'),
  publicURL: '/zhuanti/',
  script: {
    uglifyOptions: {
  ...
  envs: {
    production: {
      ...
      deployer: {
        keepLocal: true,
        handlers: [(name: string, type: string) => {
          if (type === 'html') {
...
async function uploadHtml(path: string, content: string, last = false) {
...
function uploadImage(formData): Promise<{ err: number, url: string }> {
...
function getValue(name: string, html: string) {
